...
def visualise_solution_compact(final_graph, algorithm_name, extension, q=''):
    ...
    for trajectory in final_graph.lines.trajectories:
        ...
        for connection in trajectory.connections:
            ...
            plt.plot([y1 - 0.005 * (times_visited - 1), y2 - 0.005 * (times_visited-1)], [x1 + 0.002 * (times_visited - 1), x2 +
                     0.002 * (times_visited - 1)], c=colors[n - 2], alpha=0.5, linewidth=4.0, label=f"Trajectory {n}" if c == 1 else "")

    # add titles to plot of Railway Network and remove x- and y-tics
    plt.suptitle(
        f"Railnetwork generated by {algorithm_name} Algorithm (K = {str(round(final_graph.K, 2))}, p = {str(round(len(set(final_graph.used_connections))/len(final_graph.available_connections), 2))})", fontsize=20, y=0.9)
    plt.xticks([], [])
    plt.yticks([], [])
    plt.axis('scaled')
    plt.legend()
...
def visualise_solution(final_graph, algorithm_name):
    ...
    for trajectory in final_graph.lines.trajectories:
        ...
        n_trajectories += 1
        ax.set_title(
            f'Trajectory {n_trajectories - 1}: {trajectory.stations[0]} --> {trajectory.stations[-1]} ({trajectory.duration} min.)', fontsize=25)
...
def visualise_opt_K_improvement(all_opt_K, algorithm_name, extension):
    ...
    plt.title(f"Improvement of K after {len(all_opt_K.keys())} iterations using {algorithm_name} Algorithm", fontsize=25, pad=25)
