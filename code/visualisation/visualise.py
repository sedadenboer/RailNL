...
def visualise_solution_compact(final_graph, algorithm_name, extension, q = ''):
    ...
    for trajectory in final_graph.lines.trajectories:
        ...
        for connection in trajectory.connections:
            ...
            plt.plot([y1 - 0.005 * (times_visited-1), y2 - 0.005 * (times_visited-1)], [x1 + 0.002 * (times_visited-1), x2 + 0.002 * (times_visited-1)], c = colors[n - 2], alpha = 0.5, linewidth=4.0, label=f"Trajectory {n}" if c == 1 else "")
...
def visualise_solution(final_graph, algorithm_name):
    ...
    for trajectory in final_graph.lines.trajectories:
        ...
        n_trajectories += 1
        ax.set_title(f'Trajectory {n_trajectories - 1}: {trajectory.stations[0]} --> {trajectory.stations[-1]} ({trajectory.duration} min.)', fontsize=25)
